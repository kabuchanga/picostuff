...
def smsTest(msg):
    #sendCMD_waitRespLine("AT+cmgd=,2")
    sendCMD_waitRespLine('AT+CMGF=1')
    sendCMD_waitRespLine('AT+CMGS="+254723989012"')   
    sendCMD_waitRespLine(msg)    
    sendCMD_waitRespLine("\x1A")  
    waitResp()
    
def engineerTest():
    utime.sleep(5)
    sendCMD_waitRespLine("AT+CPIN?")
    sendCMD_waitRespLine("AT+CFUN=0")        #Disable RF
    sendCMD_waitResp("AT*MCGDEFCONT=\"IP\",\""+ APN +"\"")        #Set the APN manually       
    sendCMD_waitResp("AT+CGCONTRDP")   
    sendCMD_waitResp("AT+CFUN=1\r")        #Enable RF
    sendCMD_waitResp("AT+CSMP=17,167,0,0")
    sendCMD_waitRespLine('AT+CSCA="+254722500029"') #mesaging service center no.
    #sendCMD_waitResp("AT+GMI")
    sendCMD_waitResp("AT+CGMM")
    sendCMD_waitResp("AT+CGMR")
    sendCMD_waitResp("AT+CGSN")
    sendCMD_waitResp("AT+CMGF?")
    sendCMD_waitResp("AT+CGPADDR")   #e IP address of the Mobile WAN interface
    sendCMD_waitResp("AT+CBAND?")       #Query Network information
    #utime.sleep(20)
    sendCMD_waitResp("AT+COPS?")       #Query Network information
    sendCMD_waitResp("AT+CSQ")          
    sendCMD_waitResp("AT+CGREG?")
    sendCMD_waitResp("AT+CFUN?")    
    
    sendCMD_waitResp('AT+CMGR=1') #read message 
    
    sendCMD_waitRespLine('AT+CMGF=1') #socket_id = 0,connect TCP server AT+CNMP=2
    
    sendCMD_waitRespLine('AT+CMGS="+254723989012"')   
    sendCMD_waitRespLine('This is my SMS message. Temp:' + temperature)    
    sendCMD_waitRespLine("\x1A")  
...
def callPhone():
    utime.sleep(2)
    sendCMD_waitRespLine('AT+CMGF=0')
    sendCMD_waitRespLine("ATD +254723989012")
    utime.sleep(30)
    sendCMD_waitRespLine("ATH")
    
   
    
            
#print uart info
uart = machine.UART(uart_port, uart_baute, bits=8, parity=None, stop=1,rx=Pin(9), tx=Pin(8))
print(uart)
...
while True:
    ...
    ADC_temp()
    #httpPostTest()
    smsTest('Hellow world sim7020E sms Temp: ' + str(temperature))
